...
export default function LeaguePage() {
  ...
  const simulateGame = (roundIdx: number, gameIdx: number, homeScore: number, awayScore: number) => {
    setSimulatedFixtures(
      simulatedFixtures.map((oldRound, oldRoundIdx) => oldRoundIdx === roundIdx ? oldRound.map((oldGame, oldGameIdx) => gameIdx === oldGameIdx ? ({...oldGame, homeScore, awayScore, finished: true}) : oldGame) : oldRound)
    );
  };
